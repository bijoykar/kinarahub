...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    ...
    <div class="flex items-center gap-2">
        <a href="<?= APP_URL ?>/reports/top-sellers/export/pdf?from=<?= urlencode($filters['from']) ?>&to=<?= urlencode($filters['to']) ?>" class="inline-flex items-center gap-1.5 rounded-xl bg-red-600 px-4 py-2 text-sm font-semibold text-white hover:bg-red-700 transition-colors">
        ...
        <a href="<?= APP_URL ?>/reports/top-sellers/export/csv?from=<?= urlencode($filters['from']) ?>&to=<?= urlencode($filters['to']) ?>" class="inline-flex items-center gap-1.5 rounded-xl bg-green-600 px-4 py-2 text-sm font-semibold text-white hover:bg-green-700 transition-colors">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.75"><path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3"/></svg>
            CSV
        </a>
    </div>
...
<!-- Date range filter -->
<form method="GET" action="<?= APP_URL ?>/reports/top-sellers" class="mb-6">
